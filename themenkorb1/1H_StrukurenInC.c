...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        feld[i].z = rand() % 201 - 100;
    }
    
    for (int i = 0; i < 10; i++) {
        printf("Koordinate %d: (%d, %d, %d)\n", i+1, feld[i].x, feld[i].y, feld[i].z);
    }
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = i+1; j < 10; j++) {
            double abstand = sqrt(pow(feld[j].x - feld[i].x, 2) + pow(feld[j].y - feld[i].y, 2) + pow(feld[j].z - feld[i].z, 2));
